...
def load_model(args, agg_rule):
    print('Loading the trained model...')
    attack_str = (args.attack_type + "_attacked" + str(args.num_attacked)) if args.attack else ""
    if agg_rule in ['TM', 'krum', 'multi_krum', 'bulyan', 'multi_bulyan']:
        filename = "models/Model_%s_%s_%s_%sclients_%s_%s_f%s.pt" % (args.dataset, args.iid, args.network, args.clients,
                                                                                     attack_str, agg_rule,  args.trim_size)
    elif agg_rule == 'TSM':
        trim_method = 'our'
        filename = "models/Model_%s_%s_%s_%sclients_%s_%s%s_f%s_T%s.pt" % (args.dataset, args.iid, args.network, args.clients,
                                                attack_str, agg_rule, trim_method, args.trim_size, args.temperature)
    else:
        filename = "models/Model_%s_%s_%s_%sclients_%s_%s.pt" % (args.dataset, args.iid, args.network, args.clients,
                                                               attack_str, agg_rule)
