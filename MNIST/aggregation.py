...
def multi_krum(g_list, f):
    ...
    cdist = torch.cdist(x, x, p=2)
    nbhDist, nbh = torch.topk(cdist, k + 1, largest=False)
    # print('nbhDist', nbhDist.shape, 'nbh', nbh.shape)
    ...
    # print('mkrum shape', mkrum.shape)
    # assert g_list.shape[1] == mkrum.shape

    # i_star = torch.argmin(nbhDist.sum(1))
    # mkrum = g_list[nbh[:, i_star, :].view(-1), :].mean(2, keepdims=True)
    return mkrum
