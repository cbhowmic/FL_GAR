...
def load_MNISTdataset(num_users, iidtype):
    tranform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))])
    train_dataset = datasets.MNIST('./dataset/', train=True, download=True, transform=tranform)
    test_dataset = datasets.MNIST('./dataset/', train=False, download=True, transform=tranform)
...
def get_images(dataset, indeces, batch_size, flip_signal):
